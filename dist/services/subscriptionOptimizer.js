...
exports.SubscriptionOptimizer = void 0;
class SubscriptionOptimizer {
     ...
     */
    getSubscriptionRecommendations(userPreferences) {
        let result = this.subscriptions;
        if (userPreferences.preferredCategories && userPreferences.preferredCategories.length > 0) {
            result = result.filter(sub => userPreferences.preferredCategories.some(cat => sub.name.toLowerCase().includes(cat.toLowerCase())));
        }
...
exports.SubscriptionOptimizer = SubscriptionOptimizer;
